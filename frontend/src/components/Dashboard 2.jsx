...
function Dashboard() {
  ...
  const bentoItems = [
    { label: "Total Scans", value: Math.round(animatedTotalScans), icon: "qr_code_scanner", iconColor: "text-primary" },
    { label: "E-Waste Items", value: Math.round(animatedEWaste), icon: "delete", iconColor: "text-error" },
    { label: "Non-E-Waste Items", value: Math.round(animatedNonEWaste), icon: "recycling", iconColor: "text-primary" },
    { label: "CO₂ Tracked (kg)", value: animatedCo2.toFixed(1), icon: "eco", iconColor: "text-secondary" }
  ]
          ...
          <section className="mb-12">
            <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-6" variants={stagger} initial="hidden" animate="visible">
              {bentoItems.map((item, index) => (
                <motion.div key={index} variants={fadeInUp} className="bg-surface-container-low p-6 rounded-xl animate-scale-in hover-lift card-shadow" style={{animationDelay: `${(index + 1) * 0.1}s`}}>
